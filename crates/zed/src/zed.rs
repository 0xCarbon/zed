...
fn init_reduce_motion(cx: &mut App) {
    let apply = |cx: &mut App| {
        let reduce_motion = match ReduceMotionSetting::get_global(cx).0 {
            settings::ReduceMotionMode::On => true,
            // GPUI has no OS reduced-motion detection yet, so `System` behaves
            // as `Off` until that lands.
            settings::ReduceMotionMode::System | settings::ReduceMotionMode::Off => false,
        };
